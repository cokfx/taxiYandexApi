...
<div class="feedback" id="feedback<?=$arParams['COMPONENT_ID']?>">
	...
	<form name="iblock_add" action="<?=$APPLICATION->GetCurPageParam("", array("strIMessage"), $get_index_page=false)//POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
		...
		<div class="form__header">
            <h1 class="title">Оставить отзыв о клинике</h1>
			...
			<tbody>
				...
				<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
					<tr>
							...
							<textarea placeholder=<?=$arParams["CUSTOM_TITLE_".$propertyID]?>  class="textarea form-control" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
								...
								case "N":
									...
									<?if($propertyID=='PROP_CD'){?>
										...
										<input id="<?=$propertyID.'_'.$i?>" class="inputtext <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"):?>datetime<?endif?>" type="hidden" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$_REQUEST['ELEMENT_CODE']?>" />

										<?}else{?>
								<input placeholder=<?=$arParams["CUSTOM_TITLE_".$propertyID]?> id="<?=$propertyID.'_'.$i?>" class="form-control has-clear inputtext <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"):?>datetime<?endif?>" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" />
						...
						<input type="checkbox" name="DELETE_FILE[<?=$propertyID?>][<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i?>]" id="file_delete_<?=$propertyID?>_<?=$i?>" value="Y" /><label for="file_delete_<?=$propertyID?>_<?=$i?>"><?=GetMessage("CT_EFBF_FORM_FILE_DELETE")?></label>
								...
								case "L":
									...
									<!-- <input class="niceRadio" type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="" id="property_<?=$key?>"/><label for="property_<?=$key?>"><?=GetMessage('CT_EFBF_NOT_ESTABLISHED')?></label> --><br />		
											...
											foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum)
											{
												...
												?>
								<input class="<?=$type == "checkbox" ? 'niceCheck' : 'niceRadio'?>" type="checkbox" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
												<?
											}
										break;
