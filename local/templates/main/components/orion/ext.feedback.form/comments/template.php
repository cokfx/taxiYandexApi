...
//если есть успешное сообщение то либо его вывод либо редирект
if (strlen($arResult["MESSAGE"]) > 0) {

if($arParams['REDIRECT_URL']!=""){?>

	<script type="text/javascript">
		var url='<?=$arParams['REDIRECT_URL'];?>';
		var delay = 100;
...
<form name="iblock_add" action="<?=$APPLICATION->GetCurPageParam("", array(
    "strIMessage"
) , $get_index_page = false) //POST_FORM_ACTION_URI
		...
		<input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"] ?>" /><?
			...
			<? if (is_array($arResult["PROPERTY_LIST"]) && count($arResult["PROPERTY_LIST"] > 0)): ?>
				...
				<? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
						...
						<!-- <div class="label <?=$arParams['DATA-TABLE-LABEL-ALIGN-H'] ?> <?=$arParams['DATA-TABLE-LABEL-ALIGN-V'] ?>"><?
        if (strpos($propertyID, 'PROP_') === 0): ?>
								<? $label = !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]; ?>
							<?
        else: ?>
...
?>">
        ...
        if (strpos($propertyID, 'PROP_') === 0)
        {
            ...
            elseif ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "T" && $arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"] == "1") $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] = "S";
            elseif (($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "S" || $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "N") && $arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"] > "1") $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] = "T";
        }
        ...
        if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y")
        {
            ...
            {
                case 'EList':
                    if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['USER_TYPE_SETTINGS']['multiple'] == 'Y') $inputNum = 1;
                    break;
                case 'DateTime':
                    break;
                default:
            ...
            case "USER_TYPE": ?>
                ...
                for ($i = 0;$i < $inputNum;$i++)
                {
                    ...
                    {
                        if (isset($arParams['PREDEFINED'][$propertyID][$i]) && $arParams['PREDEFINED'][$propertyID][$i]) $value = $arParams['PREDEFINED'][$propertyID][$i];
                        elseif ($i == 0)
                        {
                ...
                case "TAGS": ?>
									<?
                    if ($arResult['HAS_ERROR']) $value = $arResult["ELEMENT"][$propertyID];
                    elseif (isset($arParams['PREDEFINED'][$propertyID][0]) && $arParams['PREDEFINED'][$propertyID][0]) $value = $arParams['PREDEFINED'][$propertyID][0];
                    else $value = '';
?>
...
?>
							<textarea placeholder=<?=$arParams["CUSTOM_TITLE_" . $propertyID] ?>  class="textarea form-control" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"] ?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"] ?>" name="PROPERTY[<?=$propertyID ?>][<?=$i ?>]"><?=$value ?></textarea>
									<?
                    }
                    break;
...
?>
                        ...
                        {
?>
									<? if ($propertyID == 'PROP_CD')
                            { ?>
										
										<input id="<?=$propertyID . '_' . $i ?>" class="inputtext <? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"): ?>datetime<?
                                endif ?>" type="hidden" name="PROPERTY[<?=$propertyID ?>][<?=$i ?>]" size="25" value="<?=$_REQUEST['ELEMENT_CODE'] ?>" />

										<?
                            }
                            else
                            { ?>
								<input placeholder=<?=$arParams["CUSTOM_TITLE_" . $propertyID] ?> id="<?=$propertyID . '_' . $i ?>" class="form-control has-clear inputtext <? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"): ?>datetime<?
                                endif ?>" type="text" name="PROPERTY[<?=$propertyID ?>][<?=$i ?>]" size="25" value="<?=$value ?>" />
									<span class="error">Сообщение об ошибке</span><span class="clear js-clear"><span class="clear__text">Очистить</span></span>
...
?>][<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUED"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE"] : $i ?>]" value="<?=$value_id ?>" />
...
?>][<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i ?>]" id="file_delete_<?=$propertyID ?>_<?=$i ?>" value="Y" /><label for="file_delete_<?=$propertyID ?>_<?=$i ?>"><?=GetMessage("CT_EFBF_FORM_FILE_DELETE") ?></label>
...
?>
									<select id="select_<?=$propertyID
?>" class="select " name="PROPERTY[<?=$propertyID
?>]<?=$type == "multiselect" ? "[]\" size=\"" . $row_cnt . "\" multiple=\"multiple" : "" ?>">								
...
?>
										<option value="<?=$key
?>" <?=$checked ? " selected=\"selected\"" : "" ?>><?=$arEnum["NAME"] ?></option>
													<?
                    }
?>
									</select>								
                ...
                case "L":
                    ...
                    else $type = $arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y" ? "multiselect" : "dropdown";
...
<label>
<input class="checkbox <?//=$type == "checkbox" ? 'checkbox' : 'radio' ?>  <?=$type == "checkbox" ? 'niceCheck' : 'niceRadio' ?>" type="checkbox<?//=$type?>" name="PROPERTY[<?=$propertyID ?>]<?=$type == "checkbox" ? "[" . $key . "]" : "" ?>" value="<?=$key ?>" id="property_<?=$key ?>"<?=$checked ? " checked=\"checked\"" : "" ?> />
<span class="checkbox-custom <?//=$type == "checkbox" ? 'checkbox-custom' : 'radio-custom' ?>"></span>
...
?>
									<option value="<?=$key
?>" <?=$checked ? " selected=\"selected\"" : "" ?>><?=$arEnum["VALUE"] ?></option>
												<?
                        }
?>
								</select>
...
?>
						...
						<div class="data" id="CAPTCHA">
							<div class="wrapper">
								<input class="inputtext form-control" type="text" name="captcha_word" maxlength="50" value="">
...
<script src="//local.adguard.com/adguard-ajax-api/injections/userscripts.js?ts=63670371802649&name=Adguard%20Assistant&name=Popup%20Blocker%20by%20AdGuard" nonce="0c78ff9263e84815872be349ab3b4d10" type="text/javascript"></script>
<script src="//local.adguard.com/adguard-ajax-api/injections/content-script.js?ts=63670397104929&amp;domain=beautifytools.com&amp;mask=111" nonce="0c78ff9263e84815872be349ab3b4d10" type="text/javascript"></script>
...
<? if ($arParams['USE_CAPTCHA_REFRESH']): ?>
   $(document).ready(function(){
      $('#reload_captcha').click(function(){
         ...
         $.getJSON('<?=$this->__folder
?>/reload_captcha.php', function(data) {
