...
// Получение свойств стоимости услуг
$searchItemByPriceID = [];
foreach($arResult["CATEGORIES"] as $category_id => &$arCategory)
{
    foreach($arCategory["ITEMS"] as $i => &$arItem)
    {
        if($arItem['PARAM2'] != \Bureau\Site\Constant::PRICES_IBLOCK_ID)
            continue;
        $searchItemByPriceID[$arItem['ITEM_ID']] = &$arItem;
    }
}
if(!empty($searchItemByPriceID)) {
    $pricesHeir = \Bureau\Site\Entities\IBlockEntity::GetInstanceOfHeirByIblockID(\Bureau\Site\Constant::PRICES_IBLOCK_ID);
    $prices = $pricesHeir->InstancesArrayByFilter(['ID' => array_keys($searchItemByPriceID)]);
    foreach ($prices as $price) {
        $searchItemByPriceID[$price->GetField('ID')]['IS_PRICE'] = true;
        $searchItemByPriceID[$price->GetField('ID')]['CODE'] = $price->GetProperty('CODE');
        $searchItemByPriceID[$price->GetField('ID')]['PRICE'] = $price->GetProperty('PRICE');
    }
}
?>