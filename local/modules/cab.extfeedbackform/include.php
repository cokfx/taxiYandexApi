...
Class CCabExFeedbackForm
{
    ...
    function OnFormSubmit($FormCode = '', $bWriteToIB = true, $bSendMessage = true, $bUserSendMessage = true, $arWriteToIBParams = array(), $arSendMessageParams = array(), $arUser = array())
    {
        ...
        if (isset($arWriteToIBParams['VALUES']['PROPERTY_VALUES']['ID_ELEMENT'])) {
            $arWriteToIBParams['RESULT']['IBLOCK_ELEMENT_ID'] = $arWriteToIBParams['VALUES']['PROPERTY_VALUES']['ID_ELEMENT'];
        }
        ...
        if ($arEventParams['WRITE_TO_IB']) {
            ...
            if ($action == 'add') {
                ...
                $res = $oElement->Update($element_id, $flds, false, true, $arWriteToIBParams['PARAMS']["RESIZE_IMAGES"]);
                if (!$res) {
                    //exit($arSendMessageParams['USER_MESSAGE_ADD']);
