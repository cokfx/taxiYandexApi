...
if (CModule::IncludeModule('cab.extfeedbackform') && CModule::IncludeModule("iblock")) {
    $CabExFeedbackForm = new CCabExFeedbackForm;// in /bitrix/modules/<module>/include.php
    ...
    if ($bAllowAccess) {
        ...
        if ($arParams['IBLOCK_ELEMENT_ID']) {
            ...
            $arPropSingle = array();
            $arPropSingleSelect = array();
            $arPropMultiple = array();
            $arPropMultipleSelect = array();
    ...
    if ($bAllowAccess) {
        ...
        if (check_bitrix_sessid() && (!empty($_REQUEST["efbf_submit"]) || !empty($_REQUEST["iblock_apply"])) && $arParams['COMPONENT_ID'] == $_REQUEST['component_id']) {
            ...
            if (count($arResult["ERRORS"]) == 0 && count($arResult["MAIN_ERRORS"]) == 0) {
                    ...
                    foreach ($arFields['PROPERTY_VALUES'] as $prop => $val) {
                        ...
                        if ($prop_type == "G") {
                            ...
                            if ($arProp["MULTIPLE"] == "Y") {
                                ...
                                $_val = array();
                                foreach ($val as $vv) if ($vv = intval($vv)) $_val[] = $vv;
                                if ($_val) {
                ...
                if ($arWriteIBParams['VALUES']['PROPERTY_VALUES']['ID_ELEMENT'] > 0) {
                    $arParams['IBLOCK_ELEMENT_ID'] = intval($arWriteIBParams['VALUES']['PROPERTY_VALUES']['ID_ELEMENT']);
                    $arWriteToIBParams['RESULT']['IBLOCK_ELEMENT_ID'] = intval($arWriteIBParams['VALUES']['PROPERTY_VALUES']['ID_ELEMENT']);
                    $arMessageParams["USER_MESSAGE_ADD"] = $arParams["USER_MESSAGE_ADD"];

                }
                    //OnFormSubmit в файле include.php

                if (!$CabExFeedbackForm->OnFormSubmit($arParams['FORM_CODE'],
                    $arParams['SAVE_TO_IB'], $arParams['SEND_MESSAGE'],
                    $arParams['USER_SEND_MESSAGE'], $arWriteIBParams,
                    $arMessageParams, $arUserParams)) {
                    //  Add
                    ...
                    $arResult["MESSAGE"] = $arParams["USER_MESSAGE_ADD"];
                       ...
                       /* echo '<pre>';
                        print_r($arParams["USER_MESSAGE_ADD"]);
                        //print_r($arParams);
                        echo '</pre>';
                        die();*/

                    if ($arWriteIBParams['VALUES']['PROPERTY_VALUES']['ID_ELEMENT'] > 0) {
                        echo $arParams["USER_MESSAGE_ADD"];
