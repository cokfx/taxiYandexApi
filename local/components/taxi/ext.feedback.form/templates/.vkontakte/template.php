				...
				<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
					<tr>
						...
						[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("CT_EFBF_FORM_FILE_DOWNLOAD")?></a>]<br />
									...
									if($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y"):?>										
										<button class="button_more" onclick="return AddMoreFiles(this,'<?=$propertyID?>');"><?=GetMessage("CT_EFBF_MORE_FILES");?></button>
									<?endif;
								break;
								case "E":
								case "G":
...
?>
	...
	$(document).ready(function() {	
		...
		$('#feedback<?=$arParams['COMPONENT_ID']?> .button').bind('click', function(){
			$(this).css('text-indent', '-2000px').css('background-image', 'url(<?=$this->__folder?>/images/upload.gif)').css('background-position', 'center').css('background-repeat', 'no-repeat');
		});
		...
		$("#feedback<?=$arParams['COMPONENT_ID']?> .user-type select").each(function(){
			$(this).attr('id', 'select_' + idx++).addClass('select');
		});
