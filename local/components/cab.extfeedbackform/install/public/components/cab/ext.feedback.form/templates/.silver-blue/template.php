					...
					<tr>
						...
						<td class="data" id="<?=$propertyID?>">
							...
							switch ($INPUT_TYPE):
								...
								case "T":
									...
									?>
							<textarea class="textarea corner shadow" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
									<?
									}
								break;
								...
								case "N":
									...
									?>
									<input id="<?=$propertyID.'_'.$i?>" class="inputtext corner shadow <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"):?>datetime<?endif?>" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" /><?
						...
						[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("CT_EFBF_FORM_FILE_DOWNLOAD")?></a>]<br />
								...
								case "G":
									...
									?>
									<select id="select_<?=$propertyID?>" class="select corner shadow" name="PROPERTY[<?=$propertyID?>]<?=$type=="multiselect" ? "[]\" size=\"".$row_cnt."\" multiple=\"multiple" : ""?>">								
								...
								<input class="<?=$type == "checkbox" ? 'niceCheck' : 'niceRadio'?>" type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
										...
										case "multiselect":
										?>
								<select id="select_<?=$propertyID?>" class="select corner shadow" name="PROPERTY[<?=$propertyID?>]<?=$type=="multiselect" ? "[]\" size=\"".$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]."\" multiple=\"multiple" : ""?>">
											<?if($type == 'dropdown' && $arParams['LIST_NOT_ESTABLISHED_'.$propertyID] == 'Y'):?>	
									<option value=""><?=GetMessage('CT_EFBF_NOT_ESTABLISHED')?></option>
											<?endif;											
							...
							<div class="error-text corners <?=$err_msg_pos?> <?=$show_error?>">
								<?if($show_error):?>
									<div class="message">
					...
					<tr>
						...
						<td class="data" id="CAPTCHA">
							<div class="wrapper">
								...
								$err_msg_pos = ($arParams["ERROR_MESSAGES_POSITION"] == 'FLOAT') ? 'float': '';
								?>
								<div class="error-text err-text-corners <?=$err_msg_pos?> <?=$show_error?>">
									<?if($show_error):?>
										<div class="message">
			...
			<tfoot>
				<tr>
					<td colspan="2" class="submit-td">
						<input class="button" type="submit" name="efbf_submit" value="<?=GetMessage("CT_EFBF_FORM_SUBMIT")?>" />
					</td>
				</tr>
			</tfoot>
		</table>
...
<?
	//максимальная ширина для стилизованных элементов select (px)
	$iMaxWidth = 300; //px
?>
	<style type="text/css">	
		...
		#feedback<?=$arParams['COMPONENT_ID']?> .select, 
		#feedback<?=$arParams['COMPONENT_ID']?> .user-type select{
			max-width: <?=$iMaxWidth?>px;
			width:expression((document.documentElement.clientWidth || document.body.clientWidth) > <?=$iMaxWidth?>? "<?=$iMaxWidth?>px": "auto");		
		}

		#feedback<?=$arParams['COMPONENT_ID']?> .select .cusel-scroll-wrap, 
		#feedback<?=$arParams['COMPONENT_ID']?> .select .cuselMultiple-scroll-wrap{
			max-width: <?=$iMaxWidth?>px;
			width:expression((document.documentElement.clientWidth || document.body.clientWidth) > <?=$iMaxWidth?>? "<?=$iMaxWidth?>px": "auto");		
		}
	</style>
	...
	$(document).ready(function() {	
		...
		var idx = 1;
		$("#feedback<?=$arParams['COMPONENT_ID']?> .user-type select").each(function(){
		...
		$("#feedback<?=$arParams['COMPONENT_ID']?> .tags input").each(function(){
			$(this).addClass('corner').addClass('shadow');
		});
/*
		$("#feedback<?=$arParams['COMPONENT_ID']?> .inputtex").each(function(){
			$(this).addClass('corner').addClass('shadow');
		});
*/		
		//стилизация селектов с одиночным и множественным выбором
		$("#feedback<?=$arParams['COMPONENT_ID']?> .select").each(function(){
		...
		var idx = 100;
		$("#feedback<?=$arParams['COMPONENT_ID']?> .cusel.select").each(function(){
		...
		$("#feedback<?=$arParams['COMPONENT_ID']?> .inputtext").each(function(){
		...
		$("#feedback<?=$arParams['COMPONENT_ID']?> .textarea").each(function(){
			$(this).removeClass('corner').removeClass('shadow').addClass('corner').addClass('shadow');
		});		

		//позиционирование и анимация сообщений об ошибках
		<?if($arParams["ERROR_MESSAGES_POSITION"] == 'FLOAT'):?>
			$('#feedback<?=$arParams['COMPONENT_ID']?> .error-text.show').animate({right:"-330px"}, {queue: false, duration: 'fast'});
		<?elseif($arParams["ERROR_MESSAGES_POSITION"] == 'UNDER'):?>
			var efbf_err_h = {};
			$('#feedback<?=$arParams['COMPONENT_ID']?> .error-text.show').each(function(){
		...
		<?else:?>
			$('#feedback<?=$arParams['COMPONENT_ID']?> .error-text.show').each(function(){
				var h = $(this).height();
				$(this).height(0).animate({height: h + 'px'}, {queue: false, duration: 'fast'});
			});
		<?endif?>
		
		//реакция на нажание checkbox'а
		jQuery("#feedback<?=$arParams['COMPONENT_ID']?> .niceCheck").each(function() {		
			changeCheckStart(jQuery(this));
		});
		
		//реакция на нажание radiobutton'а
		jQuery("#feedback<?=$arParams['COMPONENT_ID']?> .niceRadio").each(function() {		
